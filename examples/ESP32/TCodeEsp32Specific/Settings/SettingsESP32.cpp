...
#include "SettingsESP32.h"
    ...
    bool SettingsESP32::init() {
        if (!SPIFFS.begin(true)) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "an Error has occurred while mounting SPIFFS\n");
            return false;
        } else {
            //LogHandler::info(SETTING_MANAGMENT_TAG, "SPIFFS mounted correctly\n");
        }
        ...
        if (!foundFile) {
            foundFile = true;
            reset();
            //LogHandler::verbose(SETTING_MANAGMENT_TAG, "setting's file not found creating settings file\n");
        } else {
            //LogHandler::verbose(SETTING_MANAGMENT_TAG, "setting's file found\n");
        }

        SettingsUsage usage;
        getSystemUsage(usage);
        //LogHandler::info(SETTING_MANAGMENT_TAG, "File System Initialised");
        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "\n================\n   System Info\n================\nSpace Available:%d\nSize of file:%d\nSize of SPIFFS:%d\n================\n", usage.spaceAvailable, usage.sizeOfFile, usage.spaceUsed);
        return true;
    }
    ...
    bool SettingsESP32::hasSetting(const char *setting) {
        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "checking cache for setting \"%s\"", setting);

        if (keyInCache(setting)) {
            //LogHandler::verbose(SETTING_MANAGMENT_TAG, "key \"%s\" in cache", setting);
            return true;
        }

        String fileData;
        if (!getFile(fileData)) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "could not load Settings file\n");
            return false;
        }
        ...
        if (error) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "an Error has occurred with the json deserialisation Error Code:%s\n", error.f_str());
            return false;
        }

        if (!doc.containsKey(setting)) {
            //LogHandler::verbose(SETTING_MANAGMENT_TAG, "setting \"%s\" not found\n", setting);
            return false;
        }
    ...
    bool SettingsESP32::getFile(String &out) {
        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "reading setting file \"%s\"\n", filepath);

        if (!isMounted()) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "failed to read.\nfilesystem is not mounted.\nMounted Status:%d\nFile Status:%d\n", fileSystemMounted, foundFile);
            return false;
        }
        ...
        if (!file) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "file could not be opened.\n");
            return false;
        }
        ...
        while (file.available()) {
            ...
            if (out.length() >= DEFAULT_JSON_FILE_SIZE) {
                //LogHandler::error(SETTING_MANAGMENT_TAG, "file is larger than %d cannot be parsed.\n", DEFAULT_JSON_FILE_SIZE);
                file.close();
                return false;
            }
        }

        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "file read successfully.\n");
        file.close();
        return true;
    }

    bool SettingsESP32::writeFile(const String &fileData) {
        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "writing setting file \"%s\"\n", filepath);

        if (!isMounted()) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "failed to write.\nfilesystem is not mounted.\nMounted Status:%d\nFile Status:%d\n", fileSystemMounted, foundFile);
            return false;
        }
        ...
        if (fileData.length() > DEFAULT_JSON_FILE_SIZE) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "file is larger than %d cannot be serialised.\n", DEFAULT_JSON_FILE_SIZE);
            return false;
        }
        ...
        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "file written successfully.\n");
        file.close();
        return true;
    }

    bool SettingsESP32::getSystemUsage(SettingsUsage &out) {
        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "getting system usage.\n");
        if (!isMounted()) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "failed to get system usage.\nfilesystem is not mounted.\nMounted Status:%d\nFile Status:%d\n", fileSystemMounted, foundFile);
            return false;
        }
        ...
        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "got system usage.\n");
        return true;
    }

    unsigned long SettingsESP32::getFileSize() {
        //LogHandler::verbose(SETTING_MANAGMENT_TAG, "getting file size.\n");
        if (!isMounted()) {
            //LogHandler::error(SETTING_MANAGMENT_TAG, "failed to get file size.\nfilesystem is not mounted.\nMounted Status:%d\nFile Status:%d\n", fileSystemMounted, foundFile);
            return 0;
        }
