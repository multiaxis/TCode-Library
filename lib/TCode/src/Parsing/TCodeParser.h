...
public:
     ...
     */
    static TCode_ChannelID constructID(const TCode_Channel_Type type, const uint8_t channel_int) // Constructs an ID from a channel type and channel number
    {
     ...
     */
    static void parseAxisCommand(const String &input, TCode_Axis_Command &out) // Parses an Axis command
    {
        bool valid = true;
        int index = 0;
        ...
        if(valid)
            out = {id, extention, rampType, command_value, command_value_extention};
    };
     ...
     */
    static void parseDeviceCommand(const String &input, TCode_Device_Command &out) // Parses a Device command
    {
        switch (toupper(input.charAt(1))) // looks at the first char and checks if it matches the values
        {
        case 'S':
            out = {TCode_Device_Command_Type::StopDevice}; return;
        case '0':
            out = {TCode_Device_Command_Type::GetSoftwareVersion}; return;
        case '1':
            out = {TCode_Device_Command_Type::GetTCodeVersion}; return;
        case '2':
            out = {TCode_Device_Command_Type::GetAssignedAxisValues}; return;
        }
     ...
     */
    static void parseSetupCommand(const String &input, TCode_Setup_Command &out) // Parses a Setup command
    {
        bool valid = true;
        int index = 1;
    ...
    static void parseExternalCommand(const String &input, TCode_External_Command &out) // Parses a Setup command
    {
        out = {input.c_str()};
    };
